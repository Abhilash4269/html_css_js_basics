...
// Below snippet is for creating html in JS

// document.body.innerHTML = `<div id="listingTable"></div>
// <div class="pagenate"> 
//     <button onclick="changePage(1)">First</button>
//     <button onclick="prevPage()" id="btn_prev">Previous</button>
//     <div id="pgNos" ></div> 
//     <button onclick="nextPage()" id="btn_next">Next</button>
//     <button onclick="changePage(numPages())">Last</button>
...
function changePage(page)  // Jump to page
{   current_page = page;
    ...
    listing_table.innerHTML = '<table class="table table-bordered" id="table" ><thead><th></th></thead><tbody></tbody></table>';
    var table_contents = document.getElementsByName("tbody")

    for (var i = (page-1) * records_per_page; i < (page * records_per_page); i++) {
        listing_table.innerHTML += `<div><b>ID: ${objJson[i].id }</b><br>Name: ${objJson[i].name }<br>Email: ${objJson[i].email }<br></div>`;
        // table_contents.innerHTML += `<td>${objJson[i].id }</td><td>${objJson[i].name }</t><th>${objJson[i].email }</th>`;
    }
    page_span.innerHTML = `Page Number :`+ page;
...
(function(){  // creates buttons automatically as self invoking function , 
    ...
    for (let i =1;i<numPages();i++){
      pgNos.innerHTML += `<div class=""><button onclick="changePage(${(i+1)})">${(i+1)}</button></div>`
    }   
})();
